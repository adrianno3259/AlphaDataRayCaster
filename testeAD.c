...
int main(int argc, char **argv)
{
    ...
    double tData[NUM_TRIS * TRI_SIZE];
    int idData[NUM_TRIS];
    double rData[NUM_RAYS * RAY_SIZE];
    int outIds[NUM_RAYS];
    double outInter[NUM_RAYS];
	...
	printf("%d - %d - %d - %d\n", sizeof(uint64_t), sizeof(uint32_t), sizeof(int*), sizeof(double*));
 
	////////////////////////////////////////////////////////////////
	////////////////////////////////////////////////////////////////

	uint64_t addr_tris = 0;
	uint64_t addr_ids = sizeof(double) * TRI_SIZE * MAX_TRIS;
	uint64_t addr_rays = addr_ids + (sizeof(int) * MAX_TRIS);
	uint64_t addr_outInter = addr_rays + (sizeof(double) * MAX_RAYS * RAY_SIZE);
	uint64_t addr_outIds = addr_outInter + (sizeof(double) * MAX_RAYS);
    
	tData[0] = 0; tData[1] = -1; tData[2] = 1;
	tData[3] = 1; tData[4] =  -1; tData[5] = -1;
	tData[6] = -1; tData[7] =  -1; tData[8] =  -1;
	...
	status = ADMXRC3_WriteDMA(hCard, 0, 0, tData, NUM_TRIS * TRI_SIZE * sizeof(double), addr_tris);
	if (status != ADMXRC3_SUCCESS) {
